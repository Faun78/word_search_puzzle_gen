...
void panic(const string &msg) {
    fail("Panic: " + msg + ". ask your code maintainer for help.");
}
...
public:
    ...
    /*
    Section I call magic I know what happens here but I know we have false positive detections here
    */
    ...
    Eventually we didn't used this function in our code
    */
...
bool generate_crossword(const int matrix_size, const vector<string> &dictionar, const string &_seed, vector<string> &solutions) {
    ...
    // Placment of secret words
    int counter =gen()% (matrix_size/words.size());
    if(counter<1){
        panic("Not enough space for words");
    }
    counter-=1;
    for (const string &word : words) {
        ...
        for (size_t i = 0; i < word.size(); i++) {
            ...
            // cerr<<random_x<<" "<<y<<" "<<word[i]<<endl; //solution
            y += gen()%increment+1;
        }
        counter+=gen()%(matrix_size/words.size())+1;
    }
...
int main(int argc, char *argv[]) {
    ...
    cin.tie(NULL);
    if (argc < 3) {
        fail("Usage: <dimensions> <seed>");
    }
    
    vector<string> solutions1;
    // Load dictionary
    const char* datasets_env = getenv("DATASET_DIR");
    if (!datasets_env) {
        datasets_env = "./";
    }
    string secrets_path = string(datasets_env) + "/secrets.txt";
    ifstream secrets_file(secrets_path);
    if (!secrets_file) {
        panic("Secrets doesn't exist");
    }
    // Load solutions
    string line;
    getline(secrets_file, line);
    int secrets_file_count = stoi(line);
    for (int i = 0; i < secrets_file_count; i++) {
        getline(secrets_file, line);
        solutions1.push_back(line);
    }

    int matrix_size = atoi(argv[1]);

    srand(strtoull(argv[2], NULL, 16) & 0x7fffffff);

    string dict_location = string(datasets_env) + "/dictionary.txt";

    ifstream spell_dictionary(dict_location);
    if (!spell_dictionary) {
        panic("Dictionary doesn't exist");
    }
    vector<string> dictionar;
	// First row has number of words so we skip it
	getline(spell_dictionary, line);
    while (getline(spell_dictionary, line)) {
        dictionar.push_back(line);
    }
    // sort dictionary firstly by word from smallest to largest and secondly by alphabetical order
    
    sort(dictionar.begin(), dictionar.end(), [](const string &a, const string &b) {
        if (a.size() == b.size()) {
            return a < b;
        }
        return a.size() < b.size();
    });
    bool ok =generate_crossword(matrix_size, dictionar, argv[2], solutions1);
